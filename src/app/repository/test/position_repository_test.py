...
def position_sample():
    return Cargo(
        id=1, nombre="Gerente de Proyecto", created_at=datetime.now(), updated_at=None
    )
...
class TestPositionRepositoryImpl:
    ...
    async def test_save_success(
        ...
        print(f"✅ Position saved successfully: ID={result.id}, Name='{result.nombre}'")
    ...
    async def test_get_all_success(self, position_repository, mock_session):
        ...
        positions = [
            Cargo(id=1, nombre="Gerente de Proyecto"),
            Cargo(id=2, nombre="Desarrollador"),
        ]
        ...
        assert len(result) == 2
        assert result[0].nombre == "Gerente de Proyecto"
        assert result[1].nombre == "Desarrollador"
        print(f"✅ All positions retrieved: {len(result)} positions found")
        for i, pos in enumerate(result):
            print(f"   - Position {i+1}: ID={pos.id}, Name='{pos.nombre}'")
    ...
    async def test_delete_success(
        ...
        print(
            f"✅ Position deleted successfully: ID=1, Name='{position_sample.nombre}'"
        )
    ...
    async def test_get_by_id_success(
        ...
        assert result == position_sample
        assert result.nombre == "Gerente de Proyecto"
        print(f"✅ Position retrieved by ID: ID={result.id}, Name='{result.nombre}'")
    ...
    async def test_get_pageable_success(self, position_repository, mock_session):
        ...
        pagina = Page(data=positions, meta=pagination_info)

        position_repository.get_pageable = AsyncMock(return_value=pagina)
    ...
    async def test_exists_by_success(self, position_repository, mock_session):
        ...
        result = await position_repository.exists_by(nombre="Gerente de Proyecto")

        assert result is True
        print(
            f"✅ Position existence verified: 'Gerente de Proyecto' exists = {result}"
        )
    ...
    async def test_exists_by_not_found(self, position_repository, mock_session):
        ...
        result = await position_repository.exists_by(nombre="Posición Inexistente")

        assert result is False
        print(
            f"✅ Position non-existence verified: 'Posición Inexistente' exists = {result}"
        )
    ...
    async def test_exists_by_invalid_field(self, position_repository):
        ...
        with pytest.raises(InvalidFieldException) as exc_info:
            await position_repository.exists_by(campo_inexistente="valor")
    ...
    async def test_find_with_filters(self, position_repository, mock_session):
        ...
        positions = [Cargo(id=1, nombre="Gerente de Proyecto")]
        ...
        position_repository.find = AsyncMock(return_value=pagina)

        search_params = {"nombre": "gerente"}
